...
_TrueWindAngle = array.array('i', (0 for _ in range(TWD_ARRAY_SIZE)))
...
class TrueWindAveraging:
    ...
    """
    def add(self, counter: int, angle:int, speed:int, twa: int):
        _TrueWindDirection[counter% TWD_ARRAY_SIZE] = angle 
        _TrueWindSpeed[counter% TWD_ARRAY_SIZE] = speed 
        _TrueWindAngle[counter% TWD_ARRAY_SIZE] = twa        
    def avgTWD(self):
    ...
    def avgTWA(self):
        return int(sum(_TrueWindAngle)/TWD_ARRAY_SIZE) 
...
class DisplayData:
    ...
    def CalcCurrentValues(self):
        ...
        awa360 = self.nmea.data_weather['wind_angle'] 
        ...
        # else:
        self.DisplayMode = "downwind" # need to refine the switch over. SDN 8/8/23
        ...
        if aws > 0.0:
            tws, twd, twa = calcTrueWind.calc(awa360, aws, self.Heading, self.BoatSpeed)
            self.trueWindAveraging.add(self._counter, twd, tws, twa)
            self.TrueWindDirection = self.trueWindAveraging.avgTWD() 
            self.TrueWindSpeed = self.trueWindAveraging.avgTWS()
            self.TrueWindAngle = self.trueWindAveraging.avgTWA()
